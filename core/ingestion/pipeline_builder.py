...
def get_source_pipeline(input_mode):
    ...
    if input_mode == INPUT_INTERNAL:
        src_pipe = f"v4l2src device={CAMERA_DEVICE} ! image/jpeg,framerate={FPS}/1,width={FRAME_X},height={FRAME_Y} ! jpegdec ! videoconvert ! video/x-raw,format=BGR ! queue leaky=downstream max-size-buffers=1 !"
    elif input_mode == INPUT_EXTERNAL:
        src_pipe = f"rtspsrc location={RTSP_URL} latency=100 drop-on-latency=true protocols=tcp ! rtph264depay ! h264parse ! avdec_h264 ! videoconvert ! videoscale ! video/x-raw,width={FRAME_X},height={FRAME_Y},format=BGR ! queue leaky=downstream max-size-buffers=1 !"
    return src_pipe
...
def get_mpegts_branch():
    return "queue leaky=downstream max-size-buffers=2 ! videoconvert ! x264enc tune=zerolatency speed-preset=ultrafast ! mpegtsmux ! tcpserversink host=0.0.0.0 port=9001 sync=false"

def get_appsink_branch():
    return f"appsink name=sink emit-signals=true drop=true sync=false max-buffers=1 "
